...
`dsm.bands` in `config/settings.yaml`. This pipeline's scorer
(`tests/score_day_schedules.py`, which every comparison and sweep here
imports) and the old pipeline's penalty report both call it, so the two
pipelines cannot be compared on different money. Check it against the
mentor's own worked example any time:
...
- **The backtest's penalty column was a placeholder until 2026-08-14**
  ...
  The pipeline-comparison numbers are unaffected: they always came from
  `tests/score_day_schedules.py`, which used the real bands.
