...
class DatasetBuilder:
    ...
    def features_for(self, captured, key):
        """
        OpenCV features for one clip, downloading it only if it is not
        already cached. Returns None when the clip is unreadable.
        """

        cache_file = FEATURE_CACHE / f"{Path(key).stem}.json"
        ...
        try:
            ...
            features = self.extractor.extract(local)
        ...
        cache_file.write_text(json.dumps(features, indent=2), encoding="utf-8")
    ...
    def build(self):
        ...
        for index, (captured, key) in enumerate(clips, start=1):
            ...
            features = self.features_for(captured, key)
            ...
            if index % 20 == 0:
                self.logger.info(
                    f"  {index}/{len(clips)} clips, {len(rows)} rows so far"
                )
        ...
        output = Path("data/windy/satellite_dataset.csv")
        ...
        print(f"horizons        : {frame['horizon_min'].min()}"
              f"-{frame['horizon_min'].max()} min")
        print(f"target_kt       : mean {frame['target_kt'].mean():.3f}, "
              f"sd {frame['target_kt'].std():.3f}")
        print(f"rows with kt_now: {int(frame['kt_now'].notna().sum())}")
        print(f"\nsaved: {output}")
...
def main():
    ...
    DatasetBuilder(args.start, args.end).build()
