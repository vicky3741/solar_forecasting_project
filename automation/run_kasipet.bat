@echo off
REM ============================================================
REM Launches the KASIPET scheduler (Windy capture + forecast + S3).
REM
...
cd /d "%~dp0.."
set SOLAR_PLANT=kasipet
set PLAYWRIGHT_BROWSERS_PATH=%LOCALAPPDATA%\ms-playwright

start /min "Solar Forecasting Scheduler - Kasipet" ".venv\Scripts\python.exe" -m modules.scheduler.scheduler
